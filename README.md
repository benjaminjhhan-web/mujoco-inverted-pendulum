...
4. My code caused the system to vibrate violently, causing the erratic oscillations. I realized that the frame rate on my simulation was too slow, causing my code to overcorrect itself. The problem was solved when I made my code take 500 snapshots per second instead of 100.
...
2. To simulate imperfect real-world hardware, I used Gaussian noise (`np.random.normal`) into the angle sensor data. The controller maintained stability despite the jittery inputs.
